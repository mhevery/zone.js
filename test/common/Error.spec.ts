...
describe('ZoneAwareError', () => {
  ...
  it('should keep error prototype chain correctly', () => {
    ...
    expect(myError instanceof (window as any)[(Zone as any).__symbol__('Error')]).toBe(true);
...
describe('Error stack', () => {
  ...
  it('Error without new which occurs in longStackTraceZone should not have zone frames and longStackTraceZone frames visible',
     (done) => {
       const CHROME_FRAMES = /^    at [\w\d.]* ?\(?.*\)?/;
       const FIRE_FOX_FRAMES = /@http/;
       const SAFARI_FRAMES = /[\w\d.]+/;
           ...
           .fork({
             ...
             onHandleError: function(
                 ...
                 boolean {
                   ...
                   else if (frames[0].indexOf('@http') !== -1)
                     frameRegexp = FIRE_FOX_FRAMES;
                   else
                     frameRegexp = SAFARI_FRAMES;
                   for (let i = 0; i < frames.length; i++) {
                     let frame = frames[i];
                     if (frame) {
